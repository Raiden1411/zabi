...
pub fn toAbiItem(self: HumanAbi, node: Node.Index) (HumanAbiErrors || error{ UnexpectedNode, UnexpectedMutability })!AbiItem {
    ...
    return switch (nodes[node]) {
        ...
        .event_proto_multi => .{ .abiEvent = try self.toAbiEventSimple(node) },
        .event_proto_simple => .{ .abiEvent = try self.toAbiEventMulti(node) },
...
pub fn toStructComponents(self: HumanAbi, node: Node.Index) HumanAbiErrors!AbiParameter {
    const nodes = self.ast.nodes.items(.tag);
    ...
    return AbiParameter{
        .type = .{ .tuple = {} },
        .name = self.ast.tokenSlice(struct_decl.name),
        .components = params,
    };
}

pub fn toStructComponentsOne(self: HumanAbi, node: Node.Index) HumanAbiErrors!AbiParameter {
    const nodes = self.ast.nodes.items(.tag);
