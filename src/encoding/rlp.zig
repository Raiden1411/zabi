...
pub const Error = Allocator.Error || Writer.Error || error{ Overflow, NegativeNumber };
...
pub fn encodeList(self: *Self, allocator: Allocator, payload: anytype) Error!void {
    ...
    switch (info) {
        ...
        .array => |arr_info| {
            ...
            var recursive: Self = .init(&alloc_writer.writer);

            for (payload) |item|
                try recursive.encodeList(allocator, item);

            const slice = try alloc_writer.toOwnedSlice();
...
/// ```
pub fn encodeRlp(allocator: Allocator, payload: anytype) Error![]u8 {
    var alloc_writer: Writer.Allocating = .init(allocator);
    defer alloc_writer.deinit();

    var encoder: Self = .init(&alloc_writer.writer);
...
/// ```
pub fn encodeRlpFromWriter(allocator: Allocator, payload: anytype, writer: *Writer) Error!void {
    var encoder: Self = .init(writer);
    try encoder.encodeList(allocator, payload);
}
