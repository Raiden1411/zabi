...
pub inline fn dupInstruction(self: *Interpreter, comptime position: u8) Interpreter.InstructionErrors!void {
    comptime std.debug.assert(position <= 16); // Size higher than expected.
...
pub inline fn pushInstruction(self: *Interpreter, comptime size: u8) Interpreter.InstructionErrors!void {
    ...
    const IntType = @Int(.unsigned, @as(u16, @intCast(size)) * 8);
...
pub inline fn swapInstruction(self: *Interpreter, comptime position: u8) Interpreter.InstructionErrors!void {
    comptime std.debug.assert(position <= 16); // Size higher than expected.
