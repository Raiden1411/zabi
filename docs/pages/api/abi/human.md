...
It uses a solidity parser + lexer combo. These are not fully compatable with the solidity ast parser.
Zabi exposes them but the recommended way to use the human readable parser is to call `parseHumanReadable`
...
const parsed = try human.parseHumanReadable(Abi, testing.allocator, slice); // [!code focus:99]
defer parsed.deinit();
```
