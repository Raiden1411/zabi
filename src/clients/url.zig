...
pub fn searchUrlParams(value: anytype, options: QueryOptions, writer: *Writer) Writer.Error!void {
    ...
    var query_writer: QueryWriter = .init(writer);

    try query_writer.beginQuery();
    inline for (info.@"struct".fields) |field| {
        try query_writer.writeParameter(field.name);
        try query_writer.writeValue(@field(value, field.name));
    }

    try query_writer.writeQueryOptions(options);
}
...
///
/// See `QueryWriter` for a more detailed documentation.
pub fn searchUrlParamsAlloc(allocator: Allocator, value: anytype, options: QueryOptions) (Allocator.Error || Writer.Error)![]u8 {
    var writer = std.Io.Writer.Allocating.init(allocator);
    errdefer writer.deinit();

    try searchUrlParams(value, options, &writer.writer);
